...
//			assert(false) : FASTQ.PARSE_CUSTOM;
		...
		if(gap==0){
			count=countFastq(fname1, fname2, k, cbits, true);
		}else{
...
//		System.out.println(Arrays.toString(freq)+"\n");
	...
	public static KCountArray countFastq(String reads1, String reads2, int k, int cbits, boolean rcomp){
		return countFastq(reads1, reads2, k, cbits, rcomp, null);
	}
	
	public static KCountArray countFastq(String reads1, String reads2, int k, int cbits, boolean rcomp, KCountArray count){
...
//					if(r!=null){KmerCount5.addRead(r, count, k, mask, rcomp);}
					KmerCount5.addRead(r, count, k, mask, rcomp);
				}
				if(r2!=null){
					if(trusted!=null){
					...
					KmerCount5.addRead(r2, count, k, mask, rcomp);
				}
...
//		System.out.println("\ncount=\n"+count);
	...
	public static KCountArray countFastqSplit(String reads1, String reads2, int k1, int k2, int gap, int cbits, boolean rcomp, KCountArray count){
		int k=k1+k2;
		assert(k<32 && k>=1 && (count!=null || k<20));
		assert(gap>=0);
