...
run_roc <- function(df, type) {
    ...
    roc_objs[['fathmm']] <- roc(df$sdv, df$fathmm_coding_score, ci = T)
    roc_objs[['cadd']] <- roc(df$sdv, df$cadd_score, ci = T)
    roc_objs[['phastCons']] <-roc(data$sdv, data$mean_phastCons_score, ci = T)
...
tiff(paste0(fig_folder,"roc_all.tiff"), width = 3.34, height = 3.44, units = 'in', res = 300)
plot_rocs(all_roc_objs, exon = F)
dev.off()

tiff(paste0(fig_folder,"roc_intron.tiff"), width = 3.34, height = 3.44, units = 'in', res = 300)
plot_rocs(intron_roc_objs, exon = F)
dev.off()

tiff(paste0(fig_folder,"roc_exon.tiff"), width = 3.34, height = 3.44, units = 'in', res = 300)
...
ggsave(paste0(fig_folder, 'snv_pr_curves_with_legend', 
              plot_format), 
...
ggsave(paste0(fig_folder, "snv_pr_curves_type", plot_format), 
       height = 4.5, width = 13.75, units = 'in', dpi = hi_res)
...
ggsave(paste0(fig_folder,"pr_curves_type", plot_format), 
