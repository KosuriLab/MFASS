...
## skipped exons ##
data_skipped <- read.table('../../../processed_data/sre/sre_data_mut_changes.txt', sep = '\t', header = T)

data_skipped$category_rename <- factor(data_skipped$category)
levels(data_skipped$category_rename) <- c('all_mut_both', 'all_mut_exon', 'all_mut_intron',
                                  'strongest_ESE', 'strongest_ESS', 'strongest_AICS',
                                  'strongest_DICS', 'consrv_1nt', 'consrv_3nt',
                                  'weaken_spurious_acceptor', 'weaken_spurious_donor', 'destroy_acceptor',
                                  'destroy_donor', 'all_mut_random_intron', 'weaken_acceptor_donor_region',
...
data_skipped$category_rename <- factor(data_skipped$category_rename,
                               levels = levels(data_skipped$category_rename)[c(34, 35, 15, 12, 13, 31, 32, 10, 11, 19, 20,
                                                                       4, 17, 5, 18, 23:26, 2, 
                                                                       6, 7, 21, 22, 27:30, 3, 14,
                                                                       1, 8, 9, 16, 33)])
...
ggsave(paste0(fig_folder, 'category_delta_index_skipped', plot_format), width = 3.5, height = 7)
