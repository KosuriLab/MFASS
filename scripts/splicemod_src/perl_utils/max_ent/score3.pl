...
die('Need at least 20 downstream and 3 upstream for MaxEnt search') if ($upstr < 20 || $downstr < 3);
...
while(<FILE>) {
	...
	if ($last_comment =~ /strand=-/) {
	    $str = reverse($str);
	    $str =~ tr/ACGTacgt/TGCAtgca/;
	}
	
	my $discard_upstr = $upstr - 20;
	my $discard_downstr = $downstr - 3;
	
	$str =~ /\w{$discard_upstr}(\w{23})\w{$discard_downstr}/;
	my $motifstr = $1;
	
	if ($usemaxent) { 
	    print sprintf(" score=%.2f\n", 
				     &log2(&scoreconsensus($motifstr)*&maxentscore(&getrest($motifstr),\@metables)));
		print $str."\n";
	}
    }
}
